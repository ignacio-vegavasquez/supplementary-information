...
WD<-"D:/IgV/Desktop/R/Normalization/Raw"
SD<-"D:/IgV/Desktop/R/Normalization/Normalized"
X0<-"Sampling Example"
...
Basal= 60 #Set the number of initial frames by which you will normalize the data (which determine F_0)
start = 180
Variable<-"Mean" #IntDen o Mean between ""
...
for(y in ToEvaluate){
  ...
  Data<-Data[start:nrow(Data),] #Delete the first "start" number of rows

  rois<-cbind(subset(Data,select=c(-1,-2)),subset(Data,select=c(-1,-2)),subset(Data,select=c(-1,-2)))
  Data<-cbind(Data[c(1,2)],rois[rep(1:(ncol(rois)/3), each = 3) + (0:2) * (ncol(rois)/3)])
  ...
  IntRaw<-grep(Variable,colnames(Data),value=TRUE) #Creates a list which include 'IntDen' and 'RawIntDen'
...
#In this line the Normalization starts
  ...
  if (substr(Answer, 1, 1) == "y"){
    ...
    # BG[,2]<-as.numeric(BG[,2])
    # BG[,3]<-as.numeric(BG[,3])
    
    Mean <- rowMeans(BG) #Creating a vector with all the average background values of all the rows
    Final<-DF[-c(Last3:number)]   #Creating dataframe which excludes the background
    ...
    names<-gsub(paste0(Variable,"."),"",names,ignore.case=T) #Subtract "IntDen" to the original name
    names<-gsub(".{1}$","",names,ignore.case=T) #Subtract the last two characters (which would be .1 .2 .3 of the triplicate)
    ...
    #Maybe you realized it is necessary to change the basal
      ...
      sample<-s[sample(length(s),9)]
      grafico<-do.call(grid.arrange, sample) #plot a grid.arrange of the plots of all the ROIs in the file
      ...
      while(loop==1){
      writeLines(" ")
      writeLines("9 rois are plotted randomly")
      
      writeLines(" ")
      RANDOM <- readline(paste(paste0('Do you want to plot another 9 rois? ',y,'/',nrow(DataBase)),'Write "y" and then press enter for continue','Anything else will continue to the saving step',' ',sep="\n")) 
      
      #Checkpoint where you decide to plot another 9 plots
      if (substr(RANDOM, 1, 1) == "y"){
        sample<-s[sample(length(s),9)]
        grafico<-do.call(grid.arrange, sample) #plot a grid.arrange of the plots of all the ROIs in the file
      }
