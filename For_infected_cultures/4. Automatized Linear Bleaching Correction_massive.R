...
#You only have to fill WD and SD paths and install the libraries if it is the first time using them

WD<-"D:/IgV/Desktop/R/Linear_bleaching_correction/Original/Bleaching mitocondria alemania/20220804"
SD<-"D:/IgV/Desktop/R/Linear_bleaching_correction/Corrected/Bleaching mitocondria alemania/20220804"

#This script will scan all the files in the folder WD, so make sure to only put a manageable number of
...
#c(1,4,7,8,9,12) to evaluate the files n� 1, 4, y7, 8, 9 and 12. c(2:5) to evaluate files 2,3,4,5.
ToEvaluate<-c(0) 
...
if(ToEvaluate[1]==0){ToEvaluate<-1:length(files)} else {}
...
for(w in ToEvaluate){
  ...
  Mean <- rowMeans(Data[,2:ncol(Data)]) #Creating a subset of DF which only contain the background ROIs
  plot(Data[,1],Mean, main='Mean of all ROIs',xlab='Time (s)',ylab='F/F0', xaxt='n')
  axis(1, at = seq(0, max(Data[,1]), by = 50), las=1)
  readline('Mean of all ROIs showed. Press any key to continue to plot ROIs individually.')
  ...
  for(g in 1:(ncol(Data)-1)){
    k<-names[g+1]
    ymax=1.5
    if(max(Data[,(g+1)])>1.5){ymax=max(Data[,(g+1)])}
    
    p[[g]]<-(ggplot(Data, aes_string(Data[,1],Data[,(g+1)])) + geom_line() 
             + labs(x = "Time (s)", y = "F/F0")+ggtitle((k))
             +theme(plot.title = element_text(hjust = 0.5))
             +scale_x_continuous(breaks = scales::pretty_breaks(n = 20)) +
              ylim(min(Data[,(g+1)]),ymax))
  }
  pages=ceiling(length(p)/9)
  for(samples in 1:pages){
  if(samples*9>length(p)){ultimo=length(p)}
    else{ultimo=samples*9}
  do.call(grid.arrange, p[(1+9*(samples-1)):ultimo]) #Plotting the ROIs as a grid arrange
  }
  print(paste0("There are ",pages," total pages of plots"))
  ...
  for (h in 2:ncol(Data)){
    ...
    if(substr(ANSWER1,1,1) == "y"){
      repeat{
        ...
        writeLines(" ")
        ANSWER3 <- readline(paste(paste0('Is it a good fitting to the curve? ',h-1,'/',ncol(Data)-1," "),' ',
                                  'Write "y" and then press enter for continue',
                                  'Anything else will be taken as a negative answer to the question',
                                  ' ',sep="\n"))
  ...
  if(substr(ANSWER6, 1, 1) == "y"){
    setwd(SD)  
    X1<-paste0(Name,' BLEACHING CORRECTED','.xlsx') #Saving the correction
    write_xlsx(Data,path=X1)
    writeLines("Bleaching correction saved")}
  
  else{
    setwd(SD) 
    X1<-paste0(Name,' NOT CORRECTED','.xlsx')
    write_xlsx(DataOrig,path=X1)
    writeLines("Bleaching correction not aplied")}}


#Script writed by Ignacio Vega-V�squez
